...
class MyConditions extends Component {
  ...
  handleClick = (e) => {
    ...
    const selectedCondition = this.props.myConditions.find(
  ...
  render() {
    console.log(
      "this is screencontent in myConditions this.props.myConditions",
      this.props.myConditions
    );

    let screenContent;

    if (this.props.myConditions) {
      screenContent = this.props.myConditions.map((condition) => (
        <h3 key={condition.id} id={condition.id} onClick={this.handleClick}>
          {" "}
          {condition.condition_name}
        </h3>
      ));
    } else {
      screenContent = "No conditions yet.";
    }
